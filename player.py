...
class Player:
    ...
    def destination_options(self, dice: int, ignore: str = None):
        ...
        ignore : str, optional
            ...
            function will be called recursively with dice=1, location
            ="Egy" and ignore="Cai", because otherwise the player would
            be able to go one step to Egypt and one step backwards,
        ...
        """
        options = []
        locs = re.split(r"-", self.location)
        # in a node
        if len(locs) == 1:
            if self.offshore:
                for x in map.sea_routes[locs[0]]:
                    if x[0] != ignore:
                        if x[1] > dice:
                            options.append(
                                locs[0]
                            ...
                            if "nd" in x[0]:
                                options = options + self.destination_options(
                                    dice - x[1], self.location
                                )
                            else:
                                options.append(x[0])
            else:
                for x in map.land_routes[locs[0]]:
                    if x[0] != ignore:
                        if x[1] > dice:
                            options.append(
                                locs[0]
